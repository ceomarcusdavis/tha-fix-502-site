        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          {support === "canceled" ? (
            <div className="mb-8 border border-border bg-surface p-5 text-sm text-muted-foreground">
              Checkout was canceled. No support payment was completed.
            </div>
          ) : null}
          ...
          <div className="border border-border bg-surface p-6 md:p-8 space-y-7">
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <button type="button" onClick={() => setCadence("one_time")} className={`border px-5 py-4 text-left transition-colors ${cadence === "one_time" ? "border-brand bg-brand/5" : "border-border hover:border-brand"}`}>
                  <div className="font-display text-xl font-bold">One-Time Support</div>
                  <div className="text-sm text-muted-foreground mt-1">One voluntary payment.</div>
                </button>
                <button type="button" onClick={() => setCadence("monthly")} className={`border px-5 py-4 text-left transition-colors ${cadence === "monthly" ? "border-brand bg-brand/5" : "border-border hover:border-brand"}`}>
                  <div className="font-display text-xl font-bold">Monthly Support</div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                {[10, 25, 50, 100].map((amt) => (
                  <button
                    ...
                    className={`border py-5 font-display text-2xl font-bold transition-colors ${!customAmount && selectedAmount === amt ? "border-brand bg-brand text-brand-foreground" : "border-border bg-background hover:border-brand"}`}
                  >
                    ${amt}
                  </button>
                ))}
              ...
              <label className="block">
                <span className="block text-xs font-bold uppercase tracking-widest mb-2">Custom amount</span>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-background border border-border pl-8 pr-4 py-4 text-sm focus:outline-none focus:border-brand"
                  />
                </div>
              </label>
            </div>
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <label>
                  <span className="block text-xs font-bold uppercase tracking-widest mb-2">Display name</span>
                  <input value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={120} placeholder="Name or organization" className="w-full bg-background border border-border px-4 py-3 text-sm focus:outline-none focus:border-brand" />
                </label>
                <label>
                  <span className="block text-xs font-bold uppercase tracking-widest mb-2">Message</span>
                  <input value={message} onChange={(event) => setMessage(event.target.value)} maxLength={1000} placeholder="Optional note to Tha Fix" className="w-full bg-background border border-border px-4 py-3 text-sm focus:outline-none focus:border-brand" />
                </label>
              </div>
              <label className="mt-4 flex items-start gap-3 text-sm">
                <input type="checkbox" checked={recognitionConsent} onChange={(event) => setRecognitionConsent(event.target.checked)} className="mt-1" />
                <span>Tha Fix may publicly recognize my support using the display name above. Leave this unchecked to remain anonymous publicly.</span>
              </label>
            </div>

            <div className="border-t border-border pt-6">
              <label className="flex items-start gap-3 text-sm leading-relaxed mb-5">
                <input type="checkbox" checked={termsAccepted} onChange={(event) => setTermsAccepted(event.target.checked)} className="mt-1" />
                <span>
                  I understand this is voluntary support to a for-profit business, is not tax-deductible, does not purchase membership or merchandise, and is governed by the <Link to="/support-terms" className="underline text-brand">Support Terms</Link>.
                </span>
              </label>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <button type="button" disabled={busy} onClick={beginCheckout} className="inline-flex items-center justify-center gap-2 bg-brand text-brand-foreground px-8 py-4 font-bold uppercase tracking-widest text-sm hover:bg-[#6A33A5] transition-colors disabled:opacity-60">
                  <Heart className="w-4 h-4 fill-current" /> {busy ? "Opening Checkout…" : `Support $${amountDollars > 0 ? amountDollars.toFixed(2) : "0.00"}${cadence === "monthly" ? "/mo" : ""}`}
                </button>
                <div className="inline-flex items-center gap-2 text-xs text-muted-foreground">
                  <ShieldCheck className="w-4 h-4 text-brand" /> Secure payment handled by Stripe
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="relative max-w-[1600px] mx-auto px-6 lg:px-10 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="text-lg max-w-xl mb-10 opacity-80">
              Membership is separate from voluntary support. Join for member benefits, early access, discussions, and the private member experience.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link to="/memberships" className="bg-white text-brand px-8 py-4 font-bold uppercase tracking-wider text-sm hover:bg-gray-100 transition-colors">See Membership Plans</Link>
              <Link to="/community" className="border border-brand-foreground/30 px-8 py-4 font-bold uppercase tracking-wider text-sm hover:bg-brand-foreground/10 transition-colors">Explore Community</Link>
            </div>
          </div>
          <div className="grid gap-4">
            {memberships.map((m) => (
              <div key={m.name} className={`p-6 border ${m.featured ? "bg-background text-foreground border-background" : "border-brand-foreground/20"}`}>
                <div className="flex items-baseline justify-between gap-4 mb-2">
                  <h3 className="font-display text-2xl font-black">{m.name}</h3>
                  <div className="font-display text-3xl font-black">${m.price}<span className={`text-xs font-medium ${m.featured ? "text-muted-foreground" : "opacity-60"}`}>{m.period}</span></div>
                </div>
      ...
      <section className="py-20 lg:py-28">
        <div className="max-w-[1600px] mx-auto px-6 lg:px-10">
          <div className="flex items-end justify-between mb-12">
            ...
            <Link to="/shop" className="hidden md:inline-flex text-xs font-bold uppercase tracking-widest border-b border-brand text-brand pb-1">Shop All</Link>
          </div>
